...
use Countable;
use Illuminate\Compatible\NamespacedItemResolver;
use Illuminate\Contracts\Loader;
use Illuminate\Contracts\Translator as TranslatorContract;
...
class Translator extends NamespacedItemResolver implements TranslatorContract
{
    ...
    {
        return Str::upper(static::substr($string, 0, 1)) . static::substr($string, 1);
    }
    ...
    protected function makeReplacements($line, array $replace)
    {
        ...
        foreach ($replace as $key => $value) {
            $line = str_replace(
                array(':' . $key, ':' . Str::upper($key), ':' . static::ucfirst($key)),
                array($value, Str::upper($value), static::ucfirst($value)),
                $line
            );
        }
    ...
    {
        $collection = new Collection($replace);

        return $collection->sortBy(function ($value, $key) {
